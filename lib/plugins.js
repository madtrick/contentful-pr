...
function triggerPluginsForHook (hookName, options, data) {
  return new Promise( function (resolve) {
    var pluginsIterator = new Iterator(plugins);
    runPlugins(pluginsIterator, hookName, resolve, options, data);
  });
}

function runPlugins (pluginsIterator, hookName, resolve, options, data) {
  ...
  if (next.done) {
    ...
    if (plugin[hookName]) {
      console.log(options);
      plugin[hookName](options, data)
      .then( function (newData) {
        runPlugins(pluginsIterator, hookName, resolve, options, newData);
      });
    } else {
      runPlugins(pluginsIterator, hookName, resolve, options, data);
    }
